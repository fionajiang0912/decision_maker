// edge-functions/api/decide.js —— EdgeOne Pages Edge Function
// 与 Vercel 版 (api/decide.js)、Cloudflare 版 (functions/api/decide.js) 功能一致：
// 收前端决策文本 -> 按 IP 每日限流 -> 调 DeepSeek -> 返回 {pros,cons,advice}
...
function getClientIp(request) {
  // EdgeOne 官方给的 eo-client-ip 或 request.eo.clientIp 最权威；退化到 x-forwarded-for
  const eoIp =
    (request.eo && request.eo.clientIp) ||
    request.headers.get("eo-client-ip");
  if (eoIp) return String(eoIp).trim();
...
// EdgeOne Pages Edge Function：POST /api/decide
